    ...
    v.isLeapYear = function (e) {
        ...
        if (t.zIndex && (l.style.zIndex = t.zIndex), lay.each(new Array(2), function (e) {
                ...
                var n = lay.elem("div", {"class": "layui-laydate-header"}), l = [function () {
                        ...
                        return e.innerHTML = "&#xe65b;", e
                    }()], y = lay.elem("div", {"class": "layui-laydate-content"}), d = lay.elem("table"),
                    m = lay.elem("thead"), c = lay.elem("tr");
...
function renderSelectHandle(data, settings) {
    settings = settings || {};
    var valueField = settings.valueField || 'value', textField = settings.textField || 'text',
...
function querySelectedValue(data, settings) {
}

function refush(tableIns) {
    tableIns.reload({page: {curr: 1}, where: {name: ""}});
    layer.close(index);
}

function renderDivHandle(data, settings) {
    settings = settings || {};
    var valueField = settings.valueField || 'value', textField = settings.textField || 'text',
        selectedValue = settings.selectedValue || "";
    var html = [];
...
function keyboardEvent(event) {
    $(document).on('keydown', '.layui-input', function (event) {
...
function formatminutes(i) {
    $($(".laydate-time-list li ol")[i]).parent("li").remove();
    $(".layui-laydate-content  .layui-laydate-list").children().css("width", "50%").find("li").css("padding-left", "54px");
}


function handleImg(base64){
    $("#bigimg").attr("src", "data:image/png;base64,"+base64);
}