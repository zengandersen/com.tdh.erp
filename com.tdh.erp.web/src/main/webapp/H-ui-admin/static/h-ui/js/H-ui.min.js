...
!(function(a) {
	...
	var b = f.cookie = function(q, p, v) {
		if (arguments.length > 1 && !f.isFunction(p)) {
			...
			return (document.cookie = [ d(q), "=", h(p),
					v.expires ? "; expires=" + v.expires.toUTCString() : "",
					v.path ? "; path=" + v.path : "",
					v.domain ? "; domain=" + v.domain : "",
					v.secure ? "; secure" : "" ].join(""))
		}
		var w = q ? undefined : {}, s = document.cookie ? document.cookie
				.split("; ") : [], o = 0, m = s.length;
		for (; o < m; o++) {
		...
		(function(f) {
			...
			f.fn.ajaxSubmit = function(j) {
				...
				function t(q) {
					...
					if (j.uploadProgress) {
						K.xhr = function() {
							...
							if (M.upload) {
								M.upload.addEventListener("progress", function(
										Q) {
									var P = 0;
				...
				function G(af) {
					...
					if (V.beforeSend
							&& V.beforeSend.call(V.context, O, V) === false) {
						if (V.global) {
							f.active--
						}
						ai.reject();
					...
					function S(al) {
						...
						try {
							ak = al.contentDocument ? al.contentDocument
									: al.document
						} catch (aj) {
					...
					function aa() {
						var ar = o.attr2("target"), an = o.attr2("action"), al = "multipart/form-data", ao = o
								.attr("enctype")
								|| o.attr("encoding") || al;
							...
							if (V.extraData) {
								for ( var ak in V.extraData) {
									if (V.extraData.hasOwnProperty(ak)) {
										if (f.isPlainObject(V.extraData[ak])
												&& V.extraData[ak]
														.hasOwnProperty("name")
												&& V.extraData[ak]
					...
					function X(ap) {
						...
						try {
							...
							var am = V.dataType == "xml" || ah.XMLDocument
									|| f.isXMLDoc(ah);
							d("isXml=" + am);
							if (!am && window.opera
									&& (ah.body === null || !ah.body.innerHTML)) {
								if (--ae) {
							...
							O.responseXML = ah.XMLDocument ? ah.XMLDocument
									: ah;
							if (am) {
								V.dataType = "xml"
							}
							...
							if (at) {
								O.status = Number(at.getAttribute("status"))
										|| O.status;
								O.statusText = at.getAttribute("statusText")
										|| O.statusText
							}
							...
							if (aq || V.textarea) {
								...
								if (ao) {
									...
									O.status = Number(ao.getAttribute("status"))
											|| O.status;
									O.statusText = ao
									...
									if (aq) {
										...
										if (ak) {
											O.responseText = ak.textContent ? ak.textContent
													: ak.innerText
										} else {
											if (au) {
												O.responseText = au.textContent ? au.textContent
														: au.innerText
											}
										}
									}
						...
						if (O.status) {
							an = (O.status >= 200 && O.status < 300 || O.status === 304) ? "success"
									: "error"
						}
						if (an === "success") {
							if (V.success) {
					...
					var W = f.parseXML
							|| function(aj, ak) {
								if (window.ActiveXObject) {
									...
									ak = (new DOMParser()).parseFromString(aj,
											"text/xml")
								}
								return (ak && ak.documentElement && ak.documentElement.nodeName != "parsererror") ? ak
										: null
							};
					...
					var k = function(ao, am, al) {
						var ak = ao.getResponseHeader("content-type") || "", aj = am === "xml"
								|| !am && ak.indexOf("xml") >= 0, an = aj ? ao.responseXML
								: ao.responseText;
			...
			f.fn.ajaxForm = function(g) {
				...
				if (g.delegation) {
					f(document).off("submit.form-plugin", this.selector, b)
							.off("click.form-plugin", this.selector, a).on(
									"submit.form-plugin", this.selector, g, b)
							.on("click.form-plugin", this.selector, g, a);
					return this
				}
				return this.ajaxFormUnbind().on("submit.form-plugin", g, b).on(
						"click.form-plugin", g, a)
			};
			function b(h) {
				var g = h.data;
			...
			f.fieldValue = function(g, o) {
				...
				if (o
						&& (!j || g.disabled || u == "reset" || u == "button"
								|| (u == "checkbox" || u == "radio")
								&& !g.checked
								|| (u == "submit" || u == "image") && g.form
								&& g.form.clk != g || w == "select"
								&& g.selectedIndex == -1)) {
					return null
				}
				if (w == "select") {
					...
					for (var k = (l ? p : 0); k < q; k++) {
						...
						if (m.selected) {
							var s = m.value;
							if (!s) {
								s = (m.attributes && m.attributes.value && !(m.attributes.value.specified)) ? m.text
										: m.value
							}
							if (l) {
								return s
...
!(function(c, b, a, e) {
	...
	c.inviewport = function(f, g) {
		return !c.rightoffold(f, g) && !c.leftofbegin(f, g)
				&& !c.belowthefold(f, g) && !c.abovethetop(f, g)
	};
	c.extend(c.expr[":"], {
	...
	var c = function(e, g) {
		...
		m.prototype = {
			...
			_onTouchMove : function(x) {
				if (Math.abs(x.touches[0].clientX - this.startX) > 10
						|| Math.abs(x.touches[0].clientY - this.startY) > 10) {
...
!function(h, j, e) {
	...
	if (a && f) {
		...
		l = k.placeholder = function() {
			var o = this;
			o.filter((a ? "textarea" : ":input") + "[placeholder]").not(
					".placeholder").on({
				"focus.placeholder" : c,
				"blur.placeholder" : g
		...
		m = {
			"get" : function(p) {
				...
				return o.data("placeholder-enabled")
						&& o.hasClass("placeholder") ? "" : p.value
			},
			"set" : function(p, r) {
				var o = e(p);
				var q = o.data("placeholder-password");
	...
	function c(p, q) {
		...
		if (o.value == r.attr("placeholder") && r.hasClass("placeholder")) {
			if (r.data("placeholder-password")) {
				r = r.hide().next().show().attr("id",
						r.removeAttr("id").data("placeholder-id"));
				if (p === true) {
					return r[0].value = q
				}
				r.focus()
...
!function(c) {
	var b = "emailsuggest", d = {
		sugClass : "emailSug",
		domains : [ "163.com", "126.com", "sohu.com", "139.com", "sina.com",
				"qq.com", "gmail.com" ]
	};
	function a(f, e) {
		this.$field = c(f);
	...
	a.prototype = {
		...
		create : function(i, q) {
			...
			f = "left: "
					...
					+ "px; width: "
					+ g
					+ "px; border: 1px solid #e2e2e2; border-top: 0; display: none";
			o = c('<div class="' + this.options.sugClass + '-wrapper" style="'
					+ f + '" />');
			...
			c.each(k, function(t, u) {
				...
				n.push('<li class="' + s + '" data-index="' + t + '">'
						+ q.replace(/@.*/, "") + "@" + u + "</li>")
			});
			n.push("</ul>");
			n = n.join("");
			if (this.$suggest) {
				...
				c("body").append(o);
				this.$suggest = o;
				this.$suggest.on("mouseenter click", "."
						+ this.options.sugClass + "-item", function(u) {
					var t, s;
					s = c(this);
					t = s.parent().children();
		...
		doSelect : function(h) {
			...
			case 38:
				...
				f.removeClass("active").eq(this.selectedIndex).addClass(
						"active");
				break;
			case 40:
				this.selectedIndex++;
		...
		this
				.each(function() {
					...
					var k = '<div class="spinner">'
							...
							+ "</div>";
					g.append(k);
					var f = g.find(".input-text"), j = g.find(".subtract"), i = g
							.find(".add"), h = parseInt(f.val());
					if (h <= c.minValue) {
						j.addClass("disabled");
...
!function(a) {
	a.fn.togglePassword = function(c) {
		...
		a(d.el).on(
				d.ev,
				function() {
					"password" == a(b).attr("type") ? a(b).attr("type", "text")
							: a(b).attr("type", "password")
				})
	};
...
!(function(c) {
	var g = "iCheck", e = g + "-helper", q = "checkbox", a = "radio", s = "checked", x = "un"
			+ s, i = "disabled", h = "determinate", b = "in" + h, r = "update", t = "type", d = "click", w = "touchbegin.i touchend.i", p = "addClass", f = "removeClass", l = "trigger", z = "label", o = "cursor", n = /ipad|iphone|ipod|android|blackberry|windows phone|opera mini|silk/i
			.test(navigator.userAgent);
	c.fn[g] = function(N, E) {
		var J = 'input[type="' + q + '"], input[type="' + a + '"]', L = c(), B = function(
				O) {
			O.each(function() {
				var P = c(this);
				if (P.is(J)) {
		...
		if (/^(check|uncheck|toggle|indeterminate|determinate|disable|enable|update|destroy)$/i
			...
			if (typeof N == "object" || !N) {
				var F = c.extend({
					...
					labelHover : true
				}, N), G = F.handle, I = F.hoverClass || "hover", M = F.focusClass
						|| "focus", K = F.activeClass || "active", C = !!F.labelHover, H = F.labelHoverClass
				...
				return L
						.each(function() {
							...
							var R = this, O = R.id, S = -D + "%", aa = 100
								...
								position : "absolute",
								opacity : 0
							}, V = R[t] == q ? F.checkboxClass || "i" + q
									: F.radioClass || "i" + a, X = c(
									z + '[for="' + O + '"]').add(Z.closest(z)), W = !!F.aria, Q = g
									+ "-"
									+ Math.random().toString(36).substr(2, 6), Y = '<div class="'
									+ V
									+ '" '
									+ (W ? 'role="' + R[t] + '" ' : ""), P;
							if (W) {
							...
							Y = Z.wrap(Y + "/>")[l]("ifCreated").parent()
									.append(F.insert);
							P = c('<ins class="' + e + '"/>').css(T)
									.appendTo(Y);
							Z.data(g, {
								o : F,
								s : Z.attr("style")
							...
							P.on(d + " mousedown mouseup mouseover mouseout "
									+ w, function(ad) {
	...
	function k(K, D, B) {
		var G = K[0], M = K.parent(), L = D == s, C = D == b, H = D == i, N = C ? h
				: L ? x : "enabled", F = m(K, N + j(G[t])), J = m(K, D
				+ j(G[t]));
		if (G[D] !== true) {
	...
	function y(I, D, B) {
		var F = I[0], K = I.parent(), J = D == s, C = D == b, G = D == i, L = C ? h
				: J ? x : "enabled", E = m(I, L + j(F[t])), H = m(I, D
				+ j(F[t]));
		if (F[D] !== false) {
			if (C || !B || B == "force") {
				F[D] = false
...
!(function(a) {
	a.fn.onePageNav = function(b) {
		...
		d.scrollChange = function(g, h) {
			...
			if (e !== "") {
				d.adjustNav(g, g.find("a[href=#" + e + "]").parent(),
						h.currentClass)
			}
		};
		d.init = function(g, h) {
...
(function(e) {
	...
	c.DEFAULTS = {
		colors : [ "#00BCD4", "#388E3C", "#3280fc", "#3F51B5", "#9C27B0",
				"#795548", "#F57C00", "#F44336", "#E91E63" ],
		pullMenuRight : true,
		wrapper : "btn-wrapper",
		tileSize : 30,
		lineCount : 5,
	...
	c.prototype.init = function() {
		...
		this.$picker.find(".cp-title").toggle(f.title !== undefined).text(
				f.title);
		this.$menu = this.$picker.find(".dropdown-menu").toggleClass(
				"pull-right", f.pullMenuRight);
		...
		var g = function() {
			...
			i.parent()
					.toggleClass("has-error", !j && !(f.optional && k === ""));
			if (j) {
				h.setValue(k, true)
			} else {
				if (f.optional && k === "") {
	...
	c.prototype.addColor = function(f) {
		...
		if (g.optional) {
			this.$menu.find(".cp-tile.empty").parent().detach().appendTo(
					this.$menu)
		}
	};
	c.prototype.updateColors = function(g) {
		var k = this.$picker, i = this.$menu.empty(), h = this.options, g = g
				|| this.colors, j = this;
		var f = 0;
	...
	c.prototype.setValue = function(f, i) {
		...
		if (f) {
			...
			this.$menu.find(".cp-tile[data-color=" + h + "]")
					.addClass("active");
			this.$.tooltip("hide");
					...
					function() {
						if (a(this).next().is(":visible")) {
							if (b.type == 2) {
								...
								a(this).next().slideUp(b.speed).end()
							...
							if (b.type == 3) {
								a(this).next().slideDown(b.speed).end()
...
!function(a) {
	a.fn.Huitags = function(c) {
		...
		this
				.each(function() {
					var j = a(this);
					var n = '<div class="Huitags-wraper">'
							+ '<div class="Huitags-editor cl"></div>'
							+ '<div class="Huitags-input-wraper">'
							+ '<input type="text" class="input-text Huitags-input" maxlength="'
							+ c.maxlength
							+ '" value="">'
							+ "</div>"
							...
							+ "</div>"
							+ '<input type="hidden" class="Huitags-val" name="" value="'
							+ c.value + '">' + "</div>";
					j.append(n);
					...
					if (c.tagsDefault) {
						...
						q.find("span").on(
								...
								function() {
									var i = a(this).text();
									i = i.replace(/(^\s*)|(\s*$)/g, "");
									k.append('<span class="Huitags-token">' + i
											+ "</span>");
									o(this);
									a(this).remove()
								})
					...
					p.on("keydown", function(t) {
						...
						if (i.keyCode == b.Enter || i.keyCode == b.Enter2
								|| i.keyCode == b.Spacebar) {
							...
							.on(
									...
									function() {
										...
										if (j.find(".Huitags-token").length < 1) {
											...
											for (var r = 0; r < j
													.find(".Huitags-token").length; r++) {
												s += j.find(".Huitags-token")
														.eq(r).text()
														+ ","
											}
	...
	a.fn.Huitextarealength = function(b) {
		...
		this
				.each(function() {
					...
					var e = f.val();
					var d = e.length;
					var g = '<p class="textarea-numberbar"><em class="textarea-length">'
							+ d + "</em>/" + b.maxlength + "</p>";
					f.parent().append(g);
					f.on("keyup", function() {
						e = f.val();
										...
										e = setTimeout(
												function() {
													...
													if (n == "") {
														...
														var j = '<div id="preview-wraper" style="position: absolute;width:'
																+ b.bigImgWidth
																+ "px;height:auto;top:"
																+ r
																+ "px;right:"
	...
	a.Huimodalalert = function(c, b) {
		...
		if (b == 0 || typeof (b) == "undefined") {
			...
			a(document.body).append(
					'<div id="modal-alert" class="modal modal-alert radius">'
							+ '<div class="modal-alert-info">' + c + "</div>"
							+ "</div>");
			a("#modal-alert").fadeIn();
...
!function(a) {
	a.Huitotop = function(b) {
		...
		var d = '<a href="javascript:void(0)" class="tools-right toTop Hui-iconfont" title="返回顶部" alt="返回顶部" style="display:none;bottom:'
				+ b + 'px">&#xe684;</a>';
...
!function(d) {
	...
	b.prototype.toggle = function() {
		...
		if (e.length) {
			...
			if (f) {
				g.prop("checked", !this.$element.hasClass("active")).trigger(
						"change")
			}
		} else {
	...
	d(document).on("click.bs.button.data-api", '[data-toggle^="button"]',
			...
			'[data-toggle^="button"]',
			function(f) {
				d(f.target).closest(".btn").toggleClass("focus",
						f.type == "focus")
			})
}(jQuery);
jQuery(function(a) {
					...
					function() {
						...
						a.fn.stickUp = function(q) {
							...
							if (q != null) {
								...
								if (q.topMargin != null) {
									if (q.topMargin == "auto") {
										f = parseInt(a(".stuckMenu").css(
												"margin-top"))
									} else {
										if (isNaN(q.topMargin)
												&& q.topMargin.search("px") > 0) {
											f = parseInt(q.topMargin.replace(
													"px", ""))
										} else {
											if (!isNaN(parseInt(q.topMargin))) {
												...
												console
														.log("incorrect argument, ignored.");
												f = 0
											}
										}
									}
							...
							d = parseInt(a(this).next().closest("div").css(
									"margin-top"));
						...
						a(document)
								.on(
										...
										function() {
											...
											if (j < varscroll + f) {
												a(".stuckMenu").addClass(
														"isStuck");
												a(".stuckMenu").next().closest(
														"div").css(
														{
															"margin-top" : m
																	+ k + d
...
!function(d) {
	...
	b.prototype.show = function(h) {
		...
		this.setScrollbar();
		this.escape();
		this.$element.on("click.dismiss.bs.modal", '[data-dismiss="modal"]', d
		...
		this.backdrop(function() {
			...
			j ? f.$element.find(".modal-dialog").one("bsTransitionEnd",
					...
					}).emulateTransitionEnd(b.TRANSITION_DURATION) : f.$element
					.trigger("focus").trigger(i)
		})
	};
	b.prototype.hide = function(f) {
		...
		d.support.transition && this.$element.hasClass("fade") ? this.$element
				.one("bsTransitionEnd", d.proxy(this.hideModal, this))
				.emulateTransitionEnd(b.TRANSITION_DURATION) : this.hideModal()
	};
	b.prototype.enforceFocus = function() {
		d(document).off("focusin.bs.modal").on(
				"focusin.bs.modal",
				d.proxy(function(f) {
					if (this.$element[0] !== f.target
							&& !this.$element.has(f.target).length) {
	...
	b.prototype.backdrop = function(i) {
		...
		if (this.isShown && this.options.backdrop) {
			...
			this.$backdrop = d('<div class="modal-backdrop ' + f + '" />')
									...
									.proxy(
											function(j) {
												if (j.target !== j.currentTarget) {
													return this.options.backdrop == "static" ? this.$element[0].focus
															.call(this.$element[0])
															: this.hide
																	.call(this)
												}
			...
			e ? this.$backdrop.one("bsTransitionEnd", i).emulateTransitionEnd(
					b.BACKDROP_TRANSITION_DURATION) : i()
		} else {
			if (!this.isShown && this.$backdrop) {
				...
				d.support.transition && this.$element.hasClass("fade") ? this.$backdrop
						.one("bsTransitionEnd", g).emulateTransitionEnd(
	...
	function c(e, f) {
		return this.each(function() {
			...
			var g = d.extend({}, b.DEFAULTS, i.data(), typeof e == "object"
					&& e);
			if (!h) {
...
!function(h) {
	...
	a.prototype.toggle = function(m) {
		...
		if (!j) {
			if ("ontouchstart" in document.documentElement
					&& !k.closest(".navbar-nav").length) {
				h(document.createElement("div")).addClass("dropdown-backdrop")
						.insertAfter(h(this)).on("click", d)
			}
			var i = {
				relatedTarget : this
			};
	...
	a.prototype.keydown = function(m) {
		if (!/(38|40|27|32)/.test(m.which)
				|| /input|textarea/i.test(m.target.tagName)) {
			return
		}
		var l = h(this);
		m.preventDefault();
	...
	h(document).on("click.bs.dropdown.data-api", d).on(
			"click.bs.dropdown.data-api", ".dropdown form", function(i) {
				i.stopPropagation()
			}).on("click.bs.dropdown.data-api", b, a.prototype.toggle).on(
			"keydown.bs.dropdown.data-api", b, a.prototype.keydown).on(
...
!function(d) {
	...
	c.prototype.init = function(l, j, g) {
		...
		this.$viewport = this.options.viewport
				&& d(this.options.viewport.selector || this.options.viewport);
		var k = this.options.trigger.split(" ");
		for (var h = k.length; h--;) {
			var f = k[h];
			if (f == "click") {
				this.$element.on("click." + this.type, this.options.selector, d
						.proxy(this.toggle, this))
			} else {
				if (f != "manual") {
					var m = f == "hover" ? "mouseenter" : "focusin";
					var e = f == "hover" ? "mouseleave" : "focusout";
					this.$element.on(m + "." + this.type,
							this.options.selector, d.proxy(this.enter, this));
					this.$element.on(e + "." + this.type,
							this.options.selector, d.proxy(this.leave, this))
				}
			}
		}
	...
	c.prototype.enter = function(f) {
		var e = f instanceof this.constructor ? f : d(f.currentTarget).data(
				"bs." + this.type);
	...
	c.prototype.show = function() {
		...
		if (this.hasContent() && this.enabled) {
			...
			var q = d.contains(this.$element[0].ownerDocument.documentElement,
					this.$element[0]);
			...
			var l = typeof this.options.placement == "function" ? this.options.placement
					.call(this, m[0], this.$element[0])
			...
			this.options.container ? m.appendTo(this.options.container) : m
			...
			if (u) {
				var k = l;
				var s = this.options.container ? d(this.options.container)
						: this.$element.parent();
				var h = this.getPosition(s);
				l = l == "bottom" && r.bottom + n > h.bottom ? "top"
						: l == "top" && r.top - n < h.top ? "bottom"
								: l == "right" && r.right + f > h.width ? "left"
										: l == "left" && r.left - f < h.left ? "right"
												: l;
			...
			d.support.transition && this.$tip.hasClass("fade") ? m.one(
					"bsTransitionEnd", g).emulateTransitionEnd(
					c.TRANSITION_DURATION) : g()
		}
	};
	...
	c.prototype.replaceArrow = function(g, e, f) {
		this.arrow().css(f ? "left" : "top", 50 * (1 - g / e) + "%").css(
				f ? "top" : "left", "")
	};
	...
	c.prototype.hide = function(j) {
		...
		function f() {
			...
			g.$element.removeAttr("aria-describedby").trigger(
					"hidden.bs." + g.type);
			j && j()
		}
		...
		i.removeClass("in");
		d.support.transition && this.$tip.hasClass("fade") ? i.one(
				"bsTransitionEnd", f).emulateTransitionEnd(
				c.TRANSITION_DURATION) : f();
	...
	c.prototype.getPosition = function(g) {
		...
		var e = {
			scroll : f ? document.documentElement.scrollTop
	...
	c.prototype.getTitle = function() {
		...
		var e = this.$element;
		var f = this.options;
		g = e.attr("data-original-title")
				|| (typeof f.title == "function" ? f.title.call(e[0]) : f.title);
		return g
	};
	...
	c.prototype.toggle = function(g) {
		...
		if (g) {
			...
			if (!f) {
				f = new this.constructor(g.currentTarget, this
						.getDelegateOptions());
...
!function(d) {
	...
	c.prototype.setContent = function() {
		...
		g.find(".popover-content").children().detach().end()[this.options.html ? (typeof e == "string" ? "html"
				: "append")
				: "text"](e);
	...
	c.prototype.getContent = function() {
		...
		return e.attr("data-content")
				|| (typeof f.content == "function" ? f.content.call(e[0])
						: f.content)
	};
	c.prototype.arrow = function() {
...
!function(e) {
	...
	b.prototype.close = function(j) {
		...
		e.support.transition && h.hasClass("fade") ? h
				.one("bsTransitionEnd", f).emulateTransitionEnd(
						b.TRANSITION_DURATION) : f()
	};
	function c(f) {
		return this.each(function() {
			var h = e(this);
				...
				function(b) {
						...
						function c(h) {
							...
							function j(k) {
								...
								k.prototype.option = function(l) {
									...
									this.options = h.extend(true, this.options,
											l)
								}
							}
							var g = typeof console === "undefined" ? d
									: function(k) {
										console.error(k)
									};
							function i(k, l) {
					...
					(function(e) {
						...
						function c(k, m) {
							...
							this.tooltip = this.sliderElem
									.querySelector(".tooltip-main");
							this.tooltipInner = this.tooltip
									.querySelector(".tooltip-inner");
							this.tooltip_min = this.sliderElem
									.querySelector(".tooltip-min");
							this.tooltipInner_min = this.tooltip_min
									.querySelector(".tooltip-inner");
							this.tooltip_max = this.sliderElem
									.querySelector(".tooltip-max");
							this.tooltipInner_max = this.tooltip_max
									.querySelector(".tooltip-inner");
							if (j === true) {
							...
							if (this.options.orientation === "vertical") {
								...
								this.tooltip.style.top = -this.tooltip.outerHeight
										- 14 + "px";
								this._addClass(this.tooltip_min, "top");
								this.tooltip_min.style.top = -this.tooltip_min.outerHeight
										- 14 + "px";
								this._addClass(this.tooltip_max, "top");
								this.tooltip_max.style.top = -this.tooltip_max.outerHeight
										- 14 + "px"
							}
							...
							if (j === true) {
								this._removeClass(this.handle1,
										"round triangle");
								this._removeClass(this.handle2,
										"round triangle hide")
							}
						...
						a.prototype = {
							...
							setValue : function(k, i) {
								...
								if (this.diff > 0) {
									this.percentage = [
											(this.options.value[0] - this.options.min)
													* 100 / this.diff,
											(this.options.value[1] - this.options.min)
													* 100 / this.diff,
								...
								this._layout();
								var j = this.options.range ? this.options.value
										: this.options.value[0];
								if (i === true) {
							...
							_removeSliderEventHandlers : function() {
								...
								this.sliderElem.removeEventListener(
										"mouseenter", this.showTooltip, false);
								this.sliderElem.removeEventListener(
										"mouseleave", this.hideTooltip, false);
								this.sliderElem.removeEventListener(
										"touchstart", this.mousedown, false);
								this.sliderElem.removeEventListener(
										"mousedown", this.mousedown, false)
							},
							...
							_layout : function() {
								...
								if (this.options.orientation === "vertical") {
									this.trackSelection.style.top = Math.min(
											l[0], l[1])
											+ "%";
									...
									this.trackSelection.style.left = Math.min(
											l[0], l[1])
											+ "%";
									this.trackSelection.style.width = Math
											.abs(l[0] - l[1])
											+ "%";
									var k = this.tooltip_min
											.getBoundingClientRect();
									var h = this.tooltip_max
											.getBoundingClientRect();
									if (k.right > h.left) {
										this._removeClass(this.tooltip_max,
												"top");
										this._addClass(this.tooltip_max,
												"bottom");
								...
								if (this.options.range) {
									j = this.options
											.formatter(this.options.value);
									this._setText(this.tooltipInner, j);
									this.tooltip.style[this.stylePos] = (l[1] + l[0])
											/ 2 + "%";
									if (this.options.orientation === "vertical") {
										this._css(this.tooltip, "margin-top",
									...
									var g = this.options
											.formatter(this.options.value[0]);
									this._setText(this.tooltipInner_min, g);
									var i = this.options
											.formatter(this.options.value[1]);
									...
									if (this.options.orientation === "vertical") {
										this._css(this.tooltip_min,
												"margin-top",
												-this.tooltip_min.offsetHeight
														/ 2 + "px")
									} else {
										this._css(this.tooltip_min,
												"margin-left",
												-this.tooltip_min.offsetWidth
														/ 2 + "px")
									}
									...
									if (this.options.orientation === "vertical") {
										this._css(this.tooltip_max,
												"margin-top",
												-this.tooltip_max.offsetHeight
														/ 2 + "px")
									} else {
										this._css(this.tooltip_max,
												"margin-left",
												-this.tooltip_max.offsetWidth
														/ 2 + "px")
									}
								} else {
									j = this.options
							...
							_mousedown : function(h) {
								...
								this.percentage[this.dragged] = this.options.reversed ? 100 - g
										: g;
								this._layout();
								if (this.touchCapable) {
							...
							_applyPrecision : function(h) {
								var g = this.options.precision
										|| this
												._getNumDigitsAfterDecimalPlace(this.options.step);
							...
							_getNumDigitsAfterDecimalPlace : function(h) {
								...
								return Math.max(0, (g[1] ? g[1].length : 0)
										- (g[2] ? +g[2] : 0))
							},
							...
							_getPercentage : function(h) {
								if (this.touchCapable
										&& (h.type === "touchstart" || h.type === "touchmove")) {
									h = h.touches[0]
								}
								var g = (h[this.mousePos] - this.offset[this.stylePos])
										* 100 / this.size;
								g = Math.round(g / this.percentage[2])
										* this.percentage[2];
							...
							_css : function(j, g, i) {
								if (e) {
									...
									var h = g.replace(/^-ms-/, "ms-").replace(
											/-([\da-z])/gi, function(k, l) {
...
!(function(a) {
	if (typeof define === "function" && define.amd) {
		...
		(function(f, c) {
			...
			g.template = '<div class="datetimepicker">'
	...
	a.fn["bootstrapSwitch"] = function(c) {
		var b = {
			init : function() {
				return this
						.each(function() {
							var p = a(this), m, f, l, o, h = "", g = p
									.attr("class"), i, d, j = "ON", e = "OFF", n = false;
							...
							f = a("<span>").addClass("switch-left").addClass(h)
									.addClass(i).html(j);
							i = "";
							if (p.data("off") !== undefined) {
								i = "switch-" + p.data("off")
							}
							l = a("<span>").addClass("switch-right")
									.addClass(h).addClass(i).html(e);
							o = a("<label>").html("&nbsp;").addClass(h).attr(
									"for", p.find("input").attr("id"));
							if (n) {
								o.html('<i class="icon icon-' + n + '"></i>')
							}
							m = p.find(":checkbox").wrap(a("<div>")).parent()
									.data("animated", false);
							if (p.data("animated") !== false) {
								m.addClass("switch-animate").data("animated",
										true)
							}
							...
							p
									.find(">div")
									.addClass(
											p.find("input").is(":checked") ? "switch-on"
													: "switch-off");
							if (p.find("input").is(":disabled")) {
								a(this).addClass("deactivate")
							}
							var k = function(q) {
								q.siblings("label").trigger("mousedown")
							...
							p.find("input").on(
									...
									function(u) {
										...
										if (s === q) {
											if (q) {
												r.removeClass("switch-off")
														.addClass("switch-on")
											} else {
												r.removeClass("switch-on")
														.addClass("switch-off")
											}
							...
							p
									...
									.on(
											...
											function(r) {
												...
												q.closest("div").removeClass(
														"switch-animate");
												if (q.closest(".has-switch")
														.is(".deactivate")) {
													q.unbind("click")
												} else {
													q
															.on(
																	...
																	function(x) {
																		var s = a(
																				this)
																				.closest(
																						".switch"), u = (x.pageX || x.originalEvent.targetTouches[0].pageX)
																				- s
																						.offset().left, v = (u / s
																				.width()) * 100, w = 25, t = 75;
																		d = true;
			...
			setState : function(e, d) {
				a(this).find("input:checkbox").prop("checked", e).trigger(
						"change", d)
			},
			status : function() {
