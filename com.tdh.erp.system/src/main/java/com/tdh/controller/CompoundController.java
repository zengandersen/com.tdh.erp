...
import com.tdh.service.CompoundService;
...
public class CompoundController extends BaseController {
    ...
    @RequestMapping(value = Route.compoundUrl.QUERY_LIST ,method= {RequestMethod.POST})
    public String list(HttpServletRequest request, Compound params){
        ...
        try{
            ...
            params.setCompound_name(searchField);
            PageList<Map<String, Object>> pageList = compoundService.selectPageMap(params);
    ...
    @RequestMapping(value = Route.compoundUrl.ADD,method ={RequestMethod.POST})
    @ResponseBody
    public String insert(HttpServletRequest request){
        ...
        try{
            ...
            //格式化
            List<Compound> list = JSONArray.toList(array, new Compound(), new JsonConfig());
            int count = compoundService.queryCountServ(login_user.getHospital_id());
            if(!CollectionUtils.isEmpty(list)){
                compoundService.mergeData(list,login_user,count);
            }
    ...
    @RequestMapping(value = Route.compoundUrl.DEL,method = {RequestMethod.POST})
    @ResponseBody
    public String delete(HttpServletRequest request){

        try{
            String compound_id = request.getParameter("compound_id");
            compoundService.deleteCompoundInfoServ(compound_id);
            return ReturnUtils.ReturnObj(ReturnUtils.ReturnParam.success,ReturnUtils.ReturnParam.success_msg,"");
        }catch(Exception e){
            return ReturnUtils.ReturnObj(ReturnUtils.ReturnParam.fail,ReturnUtils.ReturnParam.exception_msg,"");
        }
    ...
    @RequestMapping(value = Route.compoundUrl.QUERY_COMPOUNDNUM_INFO,method = {RequestMethod.POST})
    @ResponseBody
    public String query_areaenum_info(HttpServletRequest request){

        User login_user = (User) request.getSession().getAttribute(Config.loginUser);
        try{
            List<Map<String ,Object>>list = compoundService.queryCompoundEmunInfoServ(login_user.getHospital_id());
